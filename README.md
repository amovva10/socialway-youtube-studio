# YouTube Studio Dashboard

A  AI-powered YouTube Studio dashboard built with [Lovable.dev](https://lovable.dev), Supabase, and React. This app helps creators manage their content, generate ideas, and upload videos with ease — all in one place.

---

##  Features

-  YouTube dashboard interface with modern UI
-  AI-powered **Content Generator** using OpenAI
-  Upload videos directly to YouTube via OAuth
-  Supabase backend with real-time storage
-  Built using Lovable’s drag-and-prompt interface
